...
| R22 | 1.1.1 Non-text Content — OCR of scanned/image-only PDFs | A PDF detected as scanned/image-only is run through OCR (English, via ocrmypdf/tesseract) rather than rejected, so its text ships as real, extractable text instead of an inaccessible page image. Scan detection is per-page (not a document-wide average), so a single scanned page mixed into an otherwise born-digital document (e.g. a scanned signature page) still triggers OCR for the document; every page is checked individually for documents up to 50 pages, and larger documents fall back to sampling for speed, which carries the accepted tradeoff that an isolated scanned page outside the sample could be missed. The pipeline cannot verify OCR transcription accuracy, so every OCR'd document gets a document-level `OCR_APPLIED` flag naming this. Separately, each scanned page's per-page recovered-text yield is checked; a page (or the whole document) that comes back with near-zero extractable text (rotated/blank/noise scan, or a page that hit the OCR timeout) gets a distinct, louder `OCR_NO_TEXT_RECOVERED` flag making clear that page's content is not represented in the output at all, rather than being folded into the generic "verify accuracy" wording. Each page's full-page scan-raster image is excluded from the output (it's a processing artifact, not content), scoped to just the pages classified as scanned, with a single document-level `SCAN_IMAGES_EXCLUDED` info flag noting that any figures/photos embedded *within* the scan couldn't be extracted as discrete images. OCR tooling is a normal dependency of this deployment; if it's genuinely absent at runtime, the pipeline falls back to the pre-OCR rejection behavior rather than silently skipping OCR. | AUTO+FLAG (OCR applied, transcription unverifiable) / AUTO+FLAG (zero-yield pages) / INFO (scan-image exclusion) |
