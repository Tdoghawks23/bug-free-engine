...
| R8 | 1.4.5 Images of Text | Extractor does not rasterize text content into images; all text blocks from source DOCX/PDF are emitted as real text in HTML/PDF, not as image screenshots of text. Extracted images that are themselves text-as-image in the *source* document (e.g. a scanned letterhead graphic) are passed through as images with alt text obligations (R1), not converted to text — flagged AUTO+FLAG since the pipeline can't OCR. | AUTO (no new images-of-text introduced) / AUTO+FLAG (pre-existing ones flagged) |
...
| R15 | 3.1.1 Language of Page | HTML `<html lang="...">` is set from IR document language (source metadata `/Lang` for PDF, `docProps` core.xml `dc:language` for DOCX, or a deterministic langdetect fallback flagged in report); PDF catalog `/Lang` matches with a valid BCP-47 tag. Document ships REJECTED (not silently defaulted) if no language can be determined and no fallback tag is configured — this must be a hard stop, not a guess. | AUTO+FLAG (fallback-detected case) |
...
| R13b | 2.4.4 Link Purpose (In Context) | Deterministic blocklist catches known-generic phrases (R13); it cannot verify that non-generic link text ("View the 2024 report") actually matches its target or makes sense out of context. Every link is listed in the report with its text + resolved URL for human spot-check; only blocklist matches are auto-flagged as *likely* failing. | AUTO+FLAG (blocklist) / HUMAN (semantic correctness, spot-check list only) |
| R18 | 1.3.1 Info and Relationships — table complexity | Simple grid tables get `<th scope>` auto-applied (R2, AUTO). Merged/nested/irregular tables (out of scope per plan) are flagged for manual remediation rather than auto-tagged, since incorrect `scope`/`headers` attributes are worse than none. | AUTO+FLAG (simple) / HUMAN (complex, explicitly flagged "not auto-fixed") |
| R19 | 2.4.6 Headings and Labels — heading level *correctness* | Pipeline repairs level *skips* (AUTO, R14) but cannot verify a font-size-inferred heading is semantically correct (e.g. a large pull-quote misclassified as a heading in PDF extraction). All font-size-inferred (not source-tagged) headings are flagged with their confidence score for human review. | AUTO+FLAG |
| R20 | 1.3.1 / 4.1.2 — reading order on multi-column or complex-layout source PDFs | Single-column and basic two-column detection is heuristic (per plan, task 8); any page where column/order confidence is low is flagged rather than silently emitted in a possibly-wrong order. | AUTO+FLAG |
